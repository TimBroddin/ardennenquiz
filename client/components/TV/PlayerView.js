...
class Player extends Component {
    componentWillReceiveProps(nextProps) {
        if (this.props.name === nextProps.name && this.props.hasPushed !== nextProps.hasPushed && this.props.controller === nextProps.controller) {
        ...
        pushes.forEach((push, k) => {
            if (push.name === player.name) {
                ...
                if (controller && controller.start) {
                    time = ((push.date.getTime() - controller.start.getTime()) / 1000).toFixed(2);
                }
            }
        });
        playerList.push(<Player key={`player-${player.name}`} controller={controller} amount={players.length} name={player.name} score={player.score} hasPushed={hasPushed} time={time} pushIndex={pushIndex}/>);
